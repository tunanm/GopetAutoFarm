...
from Variable import (realtime_map_directory, cliff_directory,skill_press,login_screen_directory,wrong_captcha_directory,
...
if __name__ == "__main__":
    ...
    while True:
        ...
        at_login_screen = map_object_detect(login_screen_directory,realtime_map_directory)
        ...
        if len(captcha_exists) > 0:
            ...
            wrong_captcha = map_object_detect(wrong_captcha_directory,realtime_map_directory)
            if len(session_timeout) > 0:
                logout()
                re_login_and_move_to_road_to_mountain()
            elif len(wrong_captcha) > 0:
                gui.click((424,490))
            else:
                captcha_solve()
        else:
